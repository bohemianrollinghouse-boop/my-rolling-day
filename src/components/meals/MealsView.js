...
export function MealsView({
  ...
  function clearSlot(day, slot) { onUpdateMeal(day, slot, { recipeId: "" }); }
      ...
      <div className="mrd-meals-slot-card" key=${`${meal.id}-${slot}`}>
        <div className="mrd-meals-slot-head">
          <div className="mrd-meals-slot-left">
            <span className="mrd-meals-slot-icon">${slotIcon}</span>
            <span className="mrd-meals-slot-label">${slotLabel.toUpperCase()}</span>
          </div>
          <button
            className=${`mrd-meals-cook-btn ${cooked ? "on" : ""}`}
            onClick=${() => onToggleCook(meal.day, slot)}
          >
          ...
          ? html`
              <div className="mrd-meals-slot-body">
                ...
                <div className="mrd-meals-tags">
                  ...
                  ${recipe.servings ? html`<span className="mrd-meals-tag">${recipe.servings} pers.</span>` : html`<span className="mrd-meals-tag">4 pers.</span>`}
  ...
  function renderWeekOverview() {
    return html`
      <div className="mrd-meals-overview">
        ...
        ${mealRows.map((meal, idx) => {
          ...
          const lunchRecipe = meal.lunchRecipeId ? safeRecipes.find((r) => r.id === meal.lunchRecipeId) : null;
          const dinnerRecipe = meal.dinnerRecipeId ? safeRecipes.find((r) => r.id === meal.dinnerRecipeId) : null;
          const mealLabel = [lunchRecipe?.name, dinnerRecipe?.name].filter(Boolean).join(" · ");
          const isToday = idx === todayIdx;
          const isSelected = idx === selectedDayIdx;
          return html`
            <button
              key=${meal.id}
              className=${`mrd-meals-overview-row ${isToday ? "today" : ""} ${isSelected ? "selected" : ""}`}
            ...
            >
              ...
              <span className="mrd-meals-overview-meals">${mealLabel || html`<span style=${{ color: "var(--mrd-fg3)", fontStyle: "italic" }}>Rien de prévu</span>`}</span>
            </button>
          `;
        })}
      </div>
      ...
      <div className="modal-backdrop" onClick=${closePicker}>
        <div className="modal-card task-modal meal-picker-modal" onClick=${(event) => event.stopPropagation()}>
          <div className="task-modal-head">
            ...
            <button className="delbtn" onClick=${closePicker}>×</button>
          </div>
          <input
            className="ainp"
          ...
          <div className="meal-picker-list">
            ${filteredPickerRecipes.length
              ? filteredPickerRecipes.map((recipe) => html`
                  <button key=${recipe.id} type="button" className="meal-picker-card" onClick=${() => selectRecipe(recipe.id)}>
                    <div className="meal-picker-card-top">
                      ...
                      <span className="mini" style=${{ color: "var(--mrd-fg3)", flexShrink: "0", marginLeft: "8px" }}>${availabilityLabel(recipe)}</span>
                    </div>
      ...
      <div className="modal-backdrop" onClick=${() => setViewModal(null)}>
        <div className="modal-card task-modal" onClick=${(event) => event.stopPropagation()}>
          <div className="task-modal-head">
            ...
            <button className="delbtn" onClick=${() => setViewModal(null)}>×</button>
          </div>
          ${Array.isArray(recipe.labels) && recipe.labels.length
          ...
          <div className="miniTitle">Ingrédients</div>
          ${Array.isArray(recipe.ingredients) && recipe.ingredients.length
            ...
            : html`<div className="rbody" style=${{ marginBottom: "12px" }}>${recipe.ingredientsLegacy || "Non renseignés."}</div>`}
          ${Array.isArray(recipe.condiments) && recipe.condiments.length
            ? html`
                <div className="miniTitle">Condiments / épices</div>
                <div className="condiment-badge-list" style=${{ marginBottom: "16px" }}>
                  ${recipe.condiments.map((condimentId) => {
                    ...
                    return condiment ? html`<span key=${condimentId} className="condiment-badge">${condiment.label}</span>` : null;
                  })}
                </div>
              `
            : null}
          <div className="miniTitle">Préparation</div>
          <div className="rbody" style=${{ marginBottom: "16px" }}>${recipe.method || "Non renseignée."}</div>
          <div className="task-modal-actions">
            <button type="button" className="acn" onClick=${() => setViewModal(null)}>Fermer</button>
          </div>
        </div>
      </div>
    `;
    ...
    return html`
      <div className="modal-backdrop" onClick=${() => setMissingModal(null)}>
        <div className="modal-card task-modal" onClick=${(event) => event.stopPropagation()}>
          ...
          ${nothingMissing
            ? html`<div className="mini" style=${{ marginBottom: "16px", color: "var(--mrd-sage)" }}>✓ Tout est disponible dans votre inventaire.</div>`
            : null}
          ${hasIngredients ? html`
            <div className="miniTitle" style=${{ marginBottom: "6px" }}>Ingrédients manquants</div>
          ...
          ` : null}
          ${hasCondiments ? html`
            <div className="miniTitle" style=${{ marginBottom: "4px" }}>Condiments / épices</div>
            <div className="settings-stack" style=${{ gap: "6px", marginBottom: "14px" }}>
          ...
          <div className="task-modal-actions">
            ...
            ${!nothingMissing ? html`<button type="button" className="aok" onClick=${addSelectedMissingIngredients}>Ajouter à la liste</button>` : null}
          </div>
        </div>
      </div>
    `;
  ...
  return html`
    <section className="rwrap mrd-meals-wrap">
      ...
      ${/* Selected day meals */null}
      <div className="mrd-meals-day-content">
        ${selectedMeal ? html`
